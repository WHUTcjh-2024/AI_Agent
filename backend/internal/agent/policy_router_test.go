...
func TestPolicyRouterUsesWebForFreshQuestion(t *testing.T) {
	plan, err := NewPolicyRouter().Plan(context.Background(), Request{Question: "今年四六级什么时候报名？"})
	if err != nil {
		t.Fatal(err)
	}
	if plan.Route != "web_search" || plan.Search == nil || plan.Knowledge != nil {
		t.Fatalf("fresh question must route to web search: %#v", plan)
	}
}
